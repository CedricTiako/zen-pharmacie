...
import { TheTime } from '../models/time';
...
export class AdminPage implements OnInit {
  ...
  currentPharmacyGuard: PharmacyGuard;
  ...
  cuve4 = [];
  Contries: any=[];
  Region: any=['Adamaoua','Centre','Est' ,'Extrême-Nord','Littoral','Nord','Nord-Ouest','Ouest','sud','sud-ouest'];
  tmpContainElt: any[] = [];
  tmpContainElt1: any[] = [];
  codeP: any;
  cities_civ: any[]=[];
  ...
  cities: any[]=[];
  constructor(
    private ContrieServ: ContriesService,
    ...
    private apiService: ApiService,
     ...
     private cashierSevice: CashierService,
      private actionSheetController: ActionSheetController,
      private pollService: PollService,
      private partnerService: PartnerService,
       private countryService: CountryService,
       private pharmacyGuardService: PharmacyGuardService,
       private cityService: CityService
       ) { }

  getCitiesByCountriesIso2(iso2: string): any[]
  {
  ...
  ngOnInit()
   {
    this.cities=this.getCitiesByCountriesIso2('CM');
    ...
    this.partnerService.getPartnerByCountry("ci").then(
        (data) => {
         console.log("HJGFYDTSFGHJKL;JLHJ");
    ...
    //   this.cityService.getCity2().then(
    //     (data) => {
    //         console.log(data);
  ...
  loadContries()
  {
    this.ContrieServ.getAll().subscribe((res: any)=>{
      for (let index = 0; index < res.length; index++)
      {
        this.Contries[index]={
          "name": res[index].name,
          "flags": res[index].flags,
          "callingCodes": res[index].callingCodes
        };
      console.log(JSON.stringify(this.Contries));
      }
    },(err)=>
    {
  ...
  CodePays(mots: string): any
  {
    const names = this.Contries.map(el => el.name);
    console.log(this.Contries[names.indexOf(mots)]);
    this.codeP=this.Contries[names.indexOf(mots)];
    console.log(this.codeP);
    return this.Contries[names.indexOf(mots)];
  }
  onImageChange(e: any) {
  ...
  uploadImage() {
    ...
    'images/',this.imgFile ? this.sanitizer.bypassSecurityTrustResourceUrl(this.imgFile).toString() : '').then(
      (docRef: string) => {
        ...
        this.lienBase64 = this.imgFile ? this.sanitizer.bypassSecurityTrustResourceUrl(this.imgFile).toString() : '';
        if(this.listOldLink) {
           this.listOldLink += this.lienGenerer + ';';
            this.listOldBase64 += this.lienBase64;
           } else { this.listOldLink = this.lienGenerer + ';'; this.listOldBase64 += this.lienBase64; }
        localStorage.setItem('oldLinkGenered',this.listOldLink );
        localStorage.setItem('oldBase64Genered',this.listOldBase64 );
  ...
  cleanOldLink() {
    ...
    localStorage.removeItem('oldBase64Genered');
    this.listOldLink = '';
    this.lienGenerer = '';
  }
  ...
  addCampagne(form: any) {
    this.isLoading = true;
    this.campagneService.add(
      new Campagne(form.value.id,form.value.title,
        Number(form.value.status),form.value.media ? form.value.media : '',this.currentEmailUser)).then(
      () => {
        this.isLoading = false;
        this.alertService.print('Ajouter avec succes',true);
      }
    );
  }
  ...
  addArticle(form: any) {
    ...
    this.articleService.add(
      new Article(
        ...
        form.value.logo,form.value.miniatureArticle,
        form.value.categorie,form.value.tags ? form.value.tags.split(';') : [],
        form.value.link ? form.value.link : '',this.currentEmailUser)).then(
      () => {
        this.isLoading = false;
        this.alertService.print('Ajouter avec succes',true);
      }
  ...
  addFaq(form: any) {
    this.isLoading = true;
    this.faqService.add(new Faq(form.value.id,
      form.value.title,form.value.content,Number(form.value.status) as number,this.currentEmailUser)).then(
      () => {
        this.isLoading = false;
        this.alertService.print('Ajouter avec succes',true);
      }
  ...
  addCountry(form: any) {
    this.isLoading = true;
    this.countryService.add(new Country(form.value.id,
      form.value.name,Number(form.value.code) as number,form.value.flag,Number(form.value.isCountryApp),this.currentEmailUser)).then(
      () => {
        this.isLoading = false;
        this.alertService.print('Ajouter avec succes',true);
      }
  ...
  addCity(form: any) {
    this.isLoading = true;
    this.cityService.add(new City(
      form.value.name,
      form.value.countyId,
      form.value.region,
      this.currentEmailUser,null,
      form.value.long,
      form.value.lat,
      )).then(
      () => {
  ...
  addCategoryPartner(form: any) {
    this.isLoading = true;
    this.partnerService.addCategoryPartner(new CategoryPartner(form.value.id,
      form.value.name,form.value.illustration,form.value.typeCategorie,this.currentEmailUser)).then(
      () => {
        this.isLoading = false;
        this.alertService.print('Ajouter avec succes',true);
      }
  ...
  {
    console.log(e);
    this.currentPartner=e;
  }

  addPartner(form: any) {
    console.log(form.value);
    this.isLoading = true;
    this.partnerService.addPartner(new Partner(
        form.value.id,
      form.value.name,
      form.value.logo,
      form.value.idCountry,
      form.value.long ? Number(form.value.long) : 0,
      form.value.lat ? Number(form.value.lat) : 0,
       form.value.logoPins ? form.value.logoPins : '',
      form.value.subtitle ? form.value.subtitle : '',
      form.value.cover ? form.value.cover : '',
      Object.assign({},Object.assign({},(
        new PartnerDetail(form.value.contacts.split(';'),form.value.adresse ? form.value.adresse : '',
        form.value.horaire ? form.value.horaire : '', null,
        form.value.otherInfos ? form.value.otherInfos : '')))),
        
        this.tmpContainElt,Object.assign({},Object.assign({},(
          new TheTime(form.value.open_time,form.value.close_time,form.value.open_all_time)))),

          this.currentEmailUser,
          form.value.idCity)).then(
      () => {
        this.isLoading = false;
        this.alertService.print('Ajouter avec succes',true);
      }
        ...
        (err)=>{
            console.log(JSON.stringify(err));
            this.alertService.print(err.error.message,true);
        }
    );
  }
  addpharmacyGuard(form: any) {
    console.log(form.value);
    this.isLoading = true;
    this.pharmacyGuardService.addpharmacyGuard(new PharmacyGuard(
      form.value.open_day,form.value.close_day,this.currentPartner.id,null)).then(
      () => {
        this.isLoading = false;
        this.alertService.print('Ajouter avec succes',true);
      }
  ...
  addGroupePartner(form: any) {
    this.isLoading = true;
    this.partnerService.addGroupPartner(new GroupPartner(form.value.id,
      form.value.name,form.value.logo,[form.value.channel1 ? form.value.channel1 : '',
      form.value.channel2 ? form.value.channel2 : '',form.value.channel3 ? form.value.channel3 : ''],
      form.value.idFees ? form.value.idFees : '',form.value.idOperator,this.currentEmailUser)).then(
      () => {
        this.isLoading = false;
        this.alertService.print('Ajouter avec succes',true);
      }
  ...
  addCashier(form: any) {
    this.isLoading = true;
    this.cashierSevice.add(new Cashier(form.value.id,form.value.idPartner,form.value.name,
      form.value.code,Number(form.value.status),form.value.notes ? form.value.notes : '',
      form.value.media ? form.value.media : '',this.currentEmailUser)).then(
      () => {
        this.isLoading = false;
        this.alertService.print('Ajouter avec succes',true);
      }
  ...
  addPoll(form: any) {
    this.isLoading = true;
    this.pollService.addNewpoll(new Poll(form.value.id,
      form.value.title,[form.value.rep1 ? form.value.rep1.toString() : '',
      form.value.rep2 ? form.value.rep2.toString() : '',form.value.rep3 ? form.value.rep3.toString() : '',
      form.value.rep4 ? form.value.rep4.toString() : '',form.value.rep5 ? form.value.rep5.toString() : ''],
      Number(form.value.top),form.value.photo ? form.value.photo : '',Number(form.value.status),
